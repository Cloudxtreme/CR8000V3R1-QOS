...
NBB_VOID qos_show_hqos_slot()
{
    ...
    pcb = AVLL_FIND(v_spm_shared->hqos_tree, &index);

    if (NULL == pcb)
    {
        printf("can't find hqos cfg\n");
        goto EXIT_LABEL;
    }

    for (i = 0; i < ATG_DCI_HQOS_SLOT_NUM; i++)
    {
        if (0 != pcb->data->enable_switch[i].enable)
        {
...
NBB_VOID qos_show_hqos_summary()
{
    ...
    {
        if (ATG_DCI_RC_OK != v_spm_shared->hqos_lsp_pool[id]) 
        {
            lsp_num++;
        }
    }
    ...
    {
        if (ATG_DCI_RC_OK != v_spm_shared->hqos_pw_pool[id]) 
        {
            pw_num++;
        }
    }
...
NBB_VOID qos_show_usrgroup(NBB_ULONG index)
{
    ...
    pcb = AVLL_FIND(v_spm_shared->qos_user_group_tree, &index);

    if (NULL == pcb)
    {
        printf("can't find group cfg\n");
        goto EXIT_LABEL;
    }

    if (NULL == pcb->down_cfg_cb)
    {
        printf("can't find group down cfg\n");
        goto EXIT_LABEL;
    }

    printf("down policy index=%ld,port=%d\n", pcb->down_cfg_cb->qos_policy_index, pcb->port);

    if (NULL == pcb->up_cfg_cb)
    {
        printf("can't find group up cfg\n");
        goto EXIT_LABEL;
    }
    printf("up policy index=%ld\n", pcb->up_cfg_cb->qos_policy_index);
...
/*****************************************************************************
   ...
   修改内容  : 新生成函数
*****************************************************************************/
NBB_VOID qos_show_hqos_down(NBB_USHORT slot,NBB_USHORT port)
{
    SPM_HQOS_LOG_GROUP_CB *pcb = NULL;
    SPM_HQOS_LOG_USR_CB *vc_cb = NULL;
    NBB_NULL_THREAD_CONTEXT
    NBB_GET_THREAD_CONTEXT();
    ...
    for (pcb = (SPM_HQOS_LOG_GROUP_CB *)AVLL_FIRST(v_spm_shared->qos_port_cb[slot - 1][port].group_tree);
        pcb != NULL;
        pcb = (SPM_HQOS_LOG_GROUP_CB *)AVLL_NEXT(v_spm_shared->qos_port_cb[slot - 1][port].group_tree, \
                pcb->spm_hqos_group_node))
    {
        printf("lsp_id=%ld,log key=%ld,group_index=%ld,policy index=%ld\n",
            pcb->lsp_id,pcb->index, pcb->group_index, pcb->policy_index);

        for (vc_cb = (SPM_HQOS_LOG_USR_CB *)AVLL_FIRST(pcb->usr_tree); vc_cb != NULL;
             vc_cb = (SPM_HQOS_LOG_USR_CB *)AVLL_NEXT(pcb->usr_tree, vc_cb->spm_hqos_usr_node))
        {
            printf("log index=%ld,pw id=%ld,flow id=%ld,node index=%ld,policy index=%ld,que index=%ld\n",
                vc_cb->index, vc_cb->pw_id,
                vc_cb->flow_id,
                vc_cb->usr_data.node_index,
                vc_cb->usr_data.user_qos_policy_index,
                vc_cb->usr_data.prio_queue_qos_policy_index);
        }
    }
...
NBB_VOID qos_show_hqos_up(NBB_USHORT slot,NBB_USHORT port)
{
    ...
    for (pcb = (SPM_HQOS_LSP_TX_CB *)AVLL_FIRST(v_spm_shared->qos_port_cb[slot - 1][port].lsp_tree);
        pcb != NULL;
        pcb = (SPM_HQOS_LSP_TX_CB *)AVLL_NEXT(v_spm_shared->qos_port_cb[slot - 1][port].lsp_tree, \
                pcb->spm_hqos_lsp_tx_node))
    {
        printf(
            "lspkey.egr=0x%lx,lspkey.ingr=0x%lx,lspkey.lspid=%ld,lspkey.tunid=%ld,\n"
            "vrfid=%d,fec=%ld,mask=%d,type=%d,flag=%d,lsp_id=%ld,\n"
            "mode=%s,cir=%ld,pir=%ld,policy index=%ld,pw id=%ld\n",
            pcb->lsp_tx_key.tx_lsp.egress,
            pcb->lsp_tx_key.tx_lsp.ingress,
            pcb->lsp_tx_key.tx_lsp.lspid,
            pcb->lsp_tx_key.tx_lsp.tunnelid,
            pcb->lsp_tx_key.ftn.vrfid,
            pcb->lsp_tx_key.ftn.fec,
            pcb->lsp_tx_key.ftn.mask,
            pcb->lsp_tx_key.type,
            pcb->lsp_tx_key.flag,
            pcb->lsp_id,
            lsp_mode[pcb->mode],
        ...
        {
            printf("vrfkey.peerip=0x%lx,vrfkey.vrf=%d,pw id=%ld,policy index=%ld,"
                "flow index=%ld,node index=%ld,flow_id=%ld\n",
                vrf_cb->vrf_key.peer_ip,
                vrf_cb->vrf_key.vrf_id,
                vrf_cb->pw_id,
                vrf_cb->vrf_data.qos_policy_index,
                vrf_cb->vrf_data.flow_queue_qos_policy_index,
                vrf_cb->vrf_data.node_index,
                vrf_cb->flow_id);
        }
        ...
        {
            printf(
            "vckey.peerip=0x%lx,vckey.vcid=%ld,vckey.type=%d,pw id=%ld,"
            "policy index=%ld,flow index=%ld,node index=%ld,flow_id=%ld\n",
                vc_cb->vc_key.peer_ip,
                vc_cb->vc_key.vc_id,
                vc_cb->vc_key.vc_type,
                vc_cb->pw_id,
                vc_cb->vc_data.qos_policy_index,
                vc_cb->vc_data.flow_queue_qos_policy_index,
                vc_cb->vc_data.node_index,
                vc_cb->flow_id);
        }
    }
...
NBB_VOID qos_show_hqos_cfg()
{
    ...
    for(j = 0; j < MAX_SLOT_NUM;j++)
    {
        ...
        {
            for (lsp_cb = (SPM_HQOS_LSP_TX_CB *)AVLL_FIRST(v_spm_shared->qos_port_cb[j][i].lsp_tree); lsp_cb != NULL;
                 lsp_cb = (SPM_HQOS_LSP_TX_CB *)AVLL_NEXT(v_spm_shared->qos_port_cb[j][i].lsp_tree, \
                        lsp_cb->spm_hqos_lsp_tx_node))
            {
                printf(
                    "lspkey.egr=0x%lx,lspkey.ingr=0x%lx,lspkey.lspid=%ld,lspkey.tunid=%ld,lsp_id=%ld\n"
                    "vrfid=%d,fec=%ld,mask=%d,type=%d,flag=%d,node_index=%ld\n"
                    "mode=%s,cir=%ld,pir=%ld,policy index=%ld,pw id=%ld\n"
                    "port=%d,slot=%d\n\n",
                    lsp_cb->lsp_tx_key.tx_lsp.egress,
                    lsp_cb->lsp_tx_key.tx_lsp.ingress,
                    lsp_cb->lsp_tx_key.tx_lsp.lspid,
                    lsp_cb->lsp_tx_key.tx_lsp.tunnelid,
                    lsp_cb->lsp_id,
                    lsp_cb->lsp_tx_key.ftn.vrfid,
                    lsp_cb->lsp_tx_key.ftn.fec,
                    lsp_cb->lsp_tx_key.ftn.mask,
                    lsp_cb->lsp_tx_key.type,
                    lsp_cb->lsp_tx_key.flag,
                    lsp_cb->node_index,
                    lsp_mode[lsp_cb->mode],
                    lsp_cb->shaping.cir,
                    lsp_cb->shaping.pir,
                    lsp_cb->policy_index,
                    lsp_cb->pw_id,
                ...
                {
                    printf(
                    "vrfkey.peerip=0x%lx,vrfkey.vrf=%d,pw id=%ld,policy index=%ld,"
                    "flow index=%ld,node index=%ld,flow_id=%ld\n\n",
                        vrf_cb->vrf_key.peer_ip,
                        vrf_cb->vrf_key.vrf_id,
                        vrf_cb->pw_id,
            ...
            for (group_cb = (SPM_HQOS_LOG_GROUP_CB *)AVLL_FIRST(v_spm_shared->qos_port_cb[j][i].group_tree);
                group_cb != NULL;
                group_cb = (SPM_HQOS_LOG_GROUP_CB *)AVLL_NEXT(v_spm_shared->qos_port_cb[j][i].group_tree,
                    group_cb->spm_hqos_group_node))
            {
                printf("lsp_id=%ld,group key=%ld,group_index=%ld,policy index=%ld\n\n",
                    group_cb->lsp_id,group_cb->index,group_cb->group_index, group_cb->policy_index);
                ...
                {
                    printf("log index=%ld,pw id=%ld,flow id=%ld,node index=%ld,policy index=%ld,que index=%ld\n\n",
                        usr_cb->index, usr_cb->pw_id,
                        usr_cb->flow_id,
                        usr_cb->usr_data.node_index,
                        usr_cb->usr_data.user_qos_policy_index,
                        usr_cb->usr_data.prio_queue_qos_policy_index);
                }
            }
        } 
    }
...
NBB_VOID qos_show_hqos_voq(NBB_LONG slot, NBB_ULONG node_index)
{
    ...
    ret = spm_hqos_checkslot(slot, &fapid_index NBB_CCXT);
    ...
    baseQueue = fapid_index * NUM_COS + offset * node_index + HQOS_OFFSET;
...
NBB_VOID  qos_show_defend_switch(NBB_ULONG slot)
{
    ...
    pstCb = AVLL_FIND(v_spm_shared->qos_defend_tree, &slot);
...
NBB_VOID  qos_show_defend_policy(NBB_ULONG index)
{
    ...
    pstCb = AVLL_FIND(v_spm_shared->qos_defend_policy_tree, &index);
         ...
         pstAp = (SPM_QOS_DEFEND_APPERC_CB *)AVLL_NEXT(pstCb->apperc_tree, pstAp->spm_defend_apperc_node))
    {
        ...
        {
            rule_id = v_spm_shared->qos_defend_offset[pstAp->apperc_key.l3_protoc];
            num = v_spm_shared->qos_defend_num[pstAp->apperc_key.l3_protoc];
...
NBB_VOID qoshelp(NBB_VOID)
{
    ...
    static NBB_CHAR *pHelpMsg[] = {
        ...
        "qos_show_defend_policy_userdefind(index,RuleId)", 
                                                    "show userdefine cfg of defend policy",
        "qos_show_defend_policy_userdefind(index)",
                                                    "show total number of userdefine cfg",
#endif
        ...
        "---------------------new cfg print and print----------------------", "",
        "qos_set_ds_print(set)",                   "set all ds cfg print",
        "qos_set_ds_log(set)",                     "set all ds log print",
        "---------------------------DS-----------------------------------", "",
        "qos_show_ds_intf_cfg(log_key)",                "show intf ds",
        "qos_show_ds_flow_cfg(log_key)",                "show flow ds",
        "qos_show_ds_univp_cfg(log_index)",             "show univp ds",
        "qos_show_ds_vc_cfg(vc_id)",                    "show vc ds",
        "qos_show_ds_vrf_cfg(vrfid)",                   "show vrf ds",
        "qos_show_ds_ilm_cfg()",                        "show ilm ds",
        "qos_show_ds_rxlsp_cfg()",                      "show rxlsp ds",
        "qos_show_ds_tunnel_cfg()",                     "show rxlsp ds",
