...
/*****************************************************************************
   ...
   修改内容  : 新生成函数
*****************************************************************************/
SPM_QOS_USER_GROUP_CB *spm_alloc_user_group_cb(NBB_ULONG ulkey NBB_CCXT_T NBB_CXT)
{
    /***************************************************************************/
    /* Local Variables                                                         */
    /***************************************************************************/
...
/*****************************************************************************
   ...
   修改内容  : 新生成函数
*****************************************************************************/
NBB_LONG spm_free_user_group_cb(SPM_QOS_USER_GROUP_CB *pstTbl NBB_CCXT_T NBB_CXT)
{
    /***************************************************************************/
    /* Local Variables                                                         */
    /***************************************************************************/
    ...
    if(NULL != pstTbl->down_cfg_cb)
    {
       NBB_MM_FREE(pstTbl->down_cfg_cb, MEM_SPM_DOWN_USER_GROUP_CB); 
       pstTbl->down_cfg_cb = NULL;
    }
    if(NULL != pstTbl->up_cfg_cb)
    {
        NBB_MM_FREE(pstTbl->up_cfg_cb, MEM_SPM_UP_USER_GROUP_CB);
        pstTbl->up_cfg_cb = NULL;
    }

    /***************************************************************************/
    /* 现在释放单盘信息控制块。                                                */
    /***************************************************************************/
    NBB_MM_FREE(pstTbl, MEM_SPM_USER_GROUP_CB);
    pstTbl = NULL;
    ...
    NBB_BYTE *buf NBB_CCXT_T NBB_CXT)
{
    ...
    /***************************************************************************/
    NBB_LONG ret = ATG_DCI_RC_OK;
    ATG_DCI_USER_GROUP_DOWN_QOS *ptemp = NULL;
    ...
    switch (oper)
    {
        ...
        {
            ptemp = (ATG_DCI_USER_GROUP_DOWN_QOS *)buf;
            ...
            {
                pcb->down_cfg_cb = (ATG_DCI_USER_GROUP_DOWN_QOS *)NBB_MM_ALLOC(sizeof(ATG_DCI_USER_GROUP_DOWN_QOS),
                                    NBB_NORETRY_ACT, MEM_SPM_DOWN_USER_GROUP_CB);
                if (NULL == pcb->down_cfg_cb)
                {
                    ret = ATG_DCI_RC_UNSUCCESSFUL;
                    goto EXIT_LABEL;
            ...
            else
            {
                if (0 != pcb->port)
                {
                    group_cb = AVLL_FIND(SHARED.qos_port_cb[pcb->slot][pcb->port].group_tree,&(pcb->user_group_key));
                    if (NULL == group_cb)
                    {
                        ret = ATG_DCI_RC_UNSUCCESSFUL;
                        goto EXIT_LABEL;
                    }
                    ret = spm_hqos_add_log_group(group_cb->slot, group_cb->port, group_cb,
                                      ATG_DCI_RC_OK, ptemp->qos_policy_index NBB_CCXT);
                    ...
                    /* 引用计数只在出口盘 */
                    if (group_cb->slot == fapid)
                    {
                        ret = spm_disconnect_policy_cnt(group_cb->policy_index,ATG_DCI_RC_UNSUCCESSFUL NBB_CCXT);
                        if (ATG_DCI_RC_OK != ret)
                        {
                            goto EXIT_LABEL;
                        }
                    }
                    group_cb->policy_index = ptemp->qos_policy_index;
                }
            }
            OS_MEMCPY(pcb->down_cfg_cb, ptemp, sizeof(ATG_DCI_USER_GROUP_DOWN_QOS));
        }
        break;
