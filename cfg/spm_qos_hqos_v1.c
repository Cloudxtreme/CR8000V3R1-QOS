...
NBB_BYTE g_hqos_port_index[MAX_HQOS_SLOT_NUM] = {0,0,0,0};
...
static char* qos_cfg_oper_type_string[] = {"ADD", "UPD", "DEL", "INIT_ADD", "FRESH", NULL,NULL};
    ...
    const NBB_CHAR *function, NBB_ULONG line, NBB_LONG ret)
{
    ...
    if(NULL != function)
    {
        OS_PRINTF("%s line=%ld  ret=%ld ApiC3SetVpHqos voq=%ld "
                ...
                hqosEn?"使能":"不使能",(VC_BACK == proflag)?"备用":"主用");
...
/*****************************************************************************
   ...
   修改内容  : 新生成函数
*****************************************************************************/
NBB_VOID  spm_hqos_l3uni_error_log(NBB_LONG unit, NBB_ULONG l3TxUniPosId, TX_PORT_T *ptTxPort,
    NBB_LONG hqosEn, NBB_ULONG hqosFlowId,
    const NBB_CHAR *function, NBB_ULONG line, NBB_LONG ret)
{
    NBB_CHAR message[QOS_MSG_INFO_LEN];
    ...
    if(NULL != function)
    {
        printf("%s line=%ld  ret=%ld ApiC3SetL3UniHqos unit=%ld \n"
                "l3TxUniPosId=%ld,hqosEn=%s,hqosFlowId=%ld\n\n",
                function,line,ret,unit,l3TxUniPosId,hqosEn?"使能":"不使能",hqosFlowId);
...
NBB_LONG spm_hqos_del_vc_node(SPM_HQOS_LSP_CB **plsp,SPM_HQOS_VC_CB **pcb)
{
    ...
    if(NULL != vc)
    {
        spm_dbg_record_hqos_vc_head(&(vc->vc_key),SPM_OPER_DEL);
        ...
        NBB_LONG voq,NBB_LONG vc,NBB_LONG vc_id[MAX_HQOS_SLOT_NUM])
{
    ...
    else/*LSP HQOS配置存在分默认LSP节点和非默认节点之分*/
    {
        ...
        if(NULL == pcb)/*新建节点*/
        {
            ...
            ret = spm_hqos_set_vrf_drive(slot,port,pcb->pw_id,posid,data,voq,vc,vc_id,NULL);
            if(ATG_DCI_RC_OK != ret)
            {
                spm_hqos_del_vrf_node(&plsp,&pcb);
                goto EXIT_LABEL;
...
SPM_HQOS_USER_GROUP_CB *spm_alloc_hqos_usr_group_cb(NBB_ULONG index)
{
    ...
    /* Local Variables                                                         */
    /***************************************************************************/
    SPM_HQOS_USER_GROUP_CB *pstTbl = NULL;
    NBB_ULONG ret = ATG_DCI_RC_OK;
    NBB_BUF_SIZE avll_key_offset;
    ...
    pstTbl = (SPM_HQOS_USER_GROUP_CB *)NBB_MM_ALLOC(sizeof(SPM_HQOS_USER_GROUP_CB),
    ...
    /* 初始log group配置块配置条目 */
    OS_MEMSET(pstTbl, 0, sizeof(SPM_HQOS_USER_GROUP_CB));
    pstTbl->user_group_key = index;
    ...
    /* Initialize the AVLL node. */
    AVLL_INIT_NODE(pstTbl->spm_qos_user_group_node);
...
NBB_VOID spm_rcv_dci_set_user_group(ATG_DCI_SET_USER_GROUP *pst_set_ips)
{
    ...
    /***************************************************************************/
    NBB_ULONG ulkey = 0;
    SPM_HQOS_USER_GROUP_CB *pstCb = NULL;
    NBB_LONG ret = ATG_DCI_RC_OK;
    NBB_LONG rv  = ATG_DCI_RC_OK;
    ...
    if (TRUE == pst_set_ips->delete_struct)
    {
        ...
        {
            if(NULL != AVLL_FIRST(pstCb->usr_tree))   
            {
            ...
            {
               AVLL_DELETE(g_qos_user_group_tree, pstCb->spm_qos_user_group_node);
    ...
    ATG_DCI_LOG_DOWN_USER_QUEUE_QOS *data)
{
    ...
    SPM_HQOS_USER_GROUP_CB *pB = NULL;
    TX_PORT_T l3uni = {0};
    ...
    { 
        ...
        pcb = spm_alloc_hqos_log_usr_cb(logic_key,&(pB->lsp_id),slot,port);
        if (NULL == pcb)
        {
            ret = ATG_DCI_RC_UNSUCCESSFUL;
            goto EXIT_LABEL;
        }     
        ret = spm_hqos_set_usr_drive(slot,port,pB->lsp_id,pcb->pw_id,&l3uni,data,
            &(pB->data),voq,vc,vc_id,pcb,pB);
        if (ATG_DCI_RC_OK != ret) /*清除所有资源*/
        {
            spm_hqos_del_usr_node(pB,&pcb);
            spm_release_hqos_group_drive(pB);
        }
    ...
    else
    {
        ...
        /*更新到新的用户组上去了暂时不支持报错*/
        if(group_key != pB->user_group_key)
        {
            spm_qos_exception_log(__FUNCTION__,__LINE__);
            ret = ATG_DCI_RC_UNSUCCESSFUL;
            goto EXIT_LABEL;
...
NBB_VOID spm_rcv_dci_set_hqos(ATG_DCI_SET_HQOS *pstSetIps)
{
    ...
    else
    {
        ...
        ret = spm_set_hqos_slot_cfg(ulOper, pucDataStart);
